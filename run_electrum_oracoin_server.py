...
if os.path.dirname(os.path.realpath(__file__)) == os.getcwd():
    imp.load_module('electrumoracoinserver', *imp.find_module('src'))

from electrumoracoinserver import storage, networks, utils
from electrumoracoinserver.processor import Dispatcher, print_log
from electrumoracoinserver.server_processor import ServerProcessor
from electrumoracoinserver.blockchain_processor import BlockchainProcessor
from electrumoracoinserver.stratum_tcp import TcpServer
from electrumoracoinserver.stratum_http import HttpServer
...
def create_config(filename=None):
    ...
    config.set('server', 'banner', 'Welcome to Oracoin Electrum!')
    config.set('server', 'banner_file', '/etc/electrum-oracoin.banner')
    ...
    config.set('server', 'coin', 'oracoin')
    config.set('server', 'logfile', '/var/log/electrum-oracoin.log')
    config.set('server', 'donation_address', '')
    config.set('server', 'max_subscriptions', '10000')

    config.add_section('leveldb')
    config.set('leveldb', 'path', '/dev/shm/electrum-oracoin_db')
    ...
    config.set('network', 'type', 'oracoin_main')

    # try to find the config file in the default paths
    if not filename:
        for path in ('/etc/', ''):
            filename = path + 'electrum-oracoin.conf'
