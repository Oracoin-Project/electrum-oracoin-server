...
class BlockchainProcessor(Processor):
    ...
    def __init__(self, config, shared):
        ...
        self.oracoind_url = 'http://%s:%s@%s:%s/' % (
            config.get('oracoind', 'oracoind_user'),
            config.get('oracoind', 'oracoind_password'),
            config.get('oracoind', 'oracoind_host'),
            config.get('oracoind', 'oracoind_port'))
    ...
    def do_catch_up(self):
        ...
        self.header = self.block2header(self.oracoind('getblock', [self.storage.last_hash]))
        ...
        while not self.shared.stopped():
            ...
            if self.shared.paused():
                print_log("oracoind is responding")
                self.shared.unpause()
            time.sleep(10)
    ...
    def oracoind(self, method, params=[]):
        postdata = dumps({"method": method, 'params': params, 'id': 'jsonrpc'})
        while True:
            try:
                respdata = urllib.urlopen(self.oracoind_url, postdata).read()
                break
            except:
                print_log("cannot reach oracoind...")
                self.shared.pause()
                time.sleep(10)
    ...
    def get_header(self, height):
        block_hash = self.oracoind('getblockhash', [height])
        b = self.oracoind('getblock', [block_hash])
    ...
    def hash_header(self, header):
        return rev_hex(Scrypt(header_to_string(header).decode('hex')).encode('hex'))
    ...
    def get_mempool_transaction(self, txid):
        try:
            raw_tx = self.oracoind('getrawtransaction', [txid, 0])
        except:
            return None
    ...
    def import_block(self, block, block_hash, block_height, sync, revert=False):
        ...
        # add undo info
        if not revert:
            self.storage.write_undo_info(block_height, self.oracoind_height, undo_info)
    ...
    def process(self, request, cache_only=False):
        ...
        if method == 'blockchain.numblocks.subscribe':
            ...
            try:
                txo = self.oracoind('sendrawtransaction', params)
            ...
            tx_hash = params[0]
            result = self.oracoind('getrawtransaction', [tx_hash, 0])

        elif method == 'blockchain.estimatefee':
            num = int(params[0])
            result = self.oracoind('estimatefee', [num])
    ...
    def getfullblock(self, block_hash):
        block = self.oracoind('getblock', [block_hash])
        ...
        try:
            ...
            logger.error("oracoind error (getfullblock)",exc_info=True)
            self.shared.stop()
        ...
        for ir in r:
            if ir['error'] is not None:
                self.shared.stop()
                print_log("Error: make sure you run oracoind with txindex=1; use -reindex if needed.")
    ...
    def catch_up(self, sync=True):
        ...
        while not self.shared.stopped():
            ...
            # are we done yet?
            info = self.oracoind('getinfo')
            self.oracoind_height = info.get('blocks')
            oracoind_block_hash = self.oracoind('getblockhash', [self.oracoind_height])
            if self.storage.last_hash == oracoind_block_hash:
            ...
            self.up_to_date = False
            try:
                next_block_hash = self.oracoind('getblockhash', [self.storage.height + 1])
    ...
    def memorypool_update(self):
        mempool_hashes = set(self.oracoind('getrawmempool'))
